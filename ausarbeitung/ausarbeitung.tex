...
\centering
 \begin{tikzpicture}[scale=1,domain=0:4] 
    ...
    \draw[domain=0:4]    plot (\x,0)   node[right] {$G_U=G\setminus U$};
    \draw[red]    plot (\x,0.25-1/4*\x)   node[right] {$U$};
...
\end{figure}
	\item[(A3)] Wegen (P3) gibt es vier verschiedene Punkte $A,B,C,D\in\mathcal{P}$, sodass nie 3 davon auf derselben Gerade liegen. Lag also höchstens ein Punkt auf der entfernten Gerade $U$, folgt die Aussage mit den drei übrigen Punkten.\\
	Lagen zwei der vier Punkte $A,B$ auf $U$, so 
\end{enumerate}
\end{proof}
