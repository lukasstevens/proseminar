...
\begin{enumerate}
	...
	\item[(P2)] Es sei $P,Q\in\mathcal{G}$ mit $P\ne Q$. Es gibt also zwei linear unabhängige Vektoren $(a,b,c),(a',b',c')\in\ftnz$ mit:
	\begin{align*}
	P & = \{(x:y:z)\in\mathcal{P}\mid ax+by+cz=0\}\quad\normalfont{und}\\
	Q & =\{(x:y:z)\in\mathcal{P}\mid a'x+b'y+c'z=0\}
	\end{align*}
	Es folgt:
	\begin{equation*}
		P\cap Q=\{(x:y:z)\in\mathcal{P}\mid ax+by+cz=0\land a'x+b'y+c'z=0\}
	\end{equation*}
	Dieses lineare Gleichungssystem hat als Lösungsmenge einen eindimensionalen Untervektorraum, also genau einen Punkt in $\mathcal{P}$. Folglich gilt $|P\cap Q|=1$.
...
Wir wählen eine Gerade $U \in \mathcal{G}$ aus, wobei prinzipiell dazu jede Gerade gewählt werden kann. 
...
Wir wollen noch ein Lemma zeigen, dass im anschließenden Kapitel zum tragen kommt:
\begin{lem}
    Gegeben sei eine eine elliptische Kurve, die durch die Lösungen der vereinfachten Weierstraßgleichung~(\ref{weierstrass}) definiert ist:
    \begin{equation}
        E: Y^2Z = X^3 + aXZ^2 + bZ^3. 
    \end{equation}
    Es gilt: Wenn die Kurve $E$ nicht-singulär ist, dann hat das Polynom $f(x) = x^3 + ax + b$ keine mehrfachen Nullstellen.
\end{lem}
\begin{proof}
    Dazu zeigen wir, dass gilt: $E$ ist singulär $\iff$ das Polynom hat eine mehrfache Nullstelle. \\ \newline
    "$\Rightarrow$": \\
    Da die Kurve singulär ist, verschwinden die Ableitungen in mindestens einem Punkt(siehe dazu \ref{singularitaeten}).
    Wir schreiben die Weierstrassgleichung als Polynom $F(X,Y,Z) = Y^2Z - X^3 - aXZ^2 - bZ^3$, leiten ab und setzen die Ableitungen gleich null:
    \begin{equation*}
        \begin{split}
            \frac{\partial F}{\partial X} & = -3X^2 - aZ^2 = 0 \\
            \frac{\partial F}{\partial Y} & = 2YZ = 0 \\
            \frac{\partial F}{\partial Z} & = Y^2 - 2aXZ - 2bZ^2 = 0.
        \end{split}
    \end{equation*}
    Wie man sehen kann ist der unendlich ferne Punkt $\patinf = (0:1:0)$ nicht-singulär; also können wir o.B.d.A annehmen, dass wir nur affine Punkte betrachten, d.h. wir können $Z$ gleich $1$ setzen:
    \begin{equation*}
        \begin{split}
            1) \: & \frac{\partial F}{\partial x}  = -3x^2 - a = 0 \\
            2) \: & \frac{\partial F}{\partial y}  = 2y = 0 \Rightarrow y = 0 \\
            3) \: & \frac{\partial F}{\partial z}  = y^2 - 2ax - 2b = 0.
        \end{split}
    \end{equation*}
    Da $y=0$ gilt, können wir im Anschluss o.B.d.A.\ anstatt des Polynoms $f(x,y)=y^2 - x^3 - ax -b$ das Polynom $f(x)=x^3 + ax + b$ betrachten. Weiterhin wissen wir, dass es nur eine mehrfache Nullstelle geben kann, da das Polynom vom Grad 3 ist.\\ 
    $y=0$ in $3)$ einsetzen:
    \begin{equation*}
        -2ax - 3b = 0 \iff -2ax = 3b.
    \end{equation*}
    Wenn $a=0$ gilt, dann gilt auch $b=0$. Mit $1)$ folgt dann, dass die Singularität im Punkt $(0,0)$ liegt und somit hat $f(x)$ auch eine mehrfache Nullstelle in $(0,0)$, da die Funktion und die Ableitung null sind. \\
    Für $a \neq 0$ gilt $x= \frac{3b}{-2a}$. Einsetzen in $1)$:
    \begin{equation*}
        -3 \left(\frac{3b}{-2a}\right)^2 - a = 0 \iff 4a^3 + 27b^2 = 0.
    \end{equation*}
    $4a^3 + 27b^2 = 0$ ist also eine notwendige Bedingung für eine Singularität. \\
    Wir setzen $f(x) = 0$ mit $x= \frac{3b}{-2a}$:
    \begin{equation*}
            \left(\frac{3b}{-2a}\right)^3 + a \left(\frac{3b}{-2a}\right) + b = 0 \iff b(27b^2 - 4a^3) = 0
    \end{equation*}
    Es gibt also auch immer eine Nullstelle an der Stelle der Singularität, falls $4a^3 + 27b^2 = 0$ gilt und diese Nullstelle ist dann auch eine mehrfachen Nullstelle.
    Daraus folgt, dass das Polynom $f(x)$ eine mehrfache Nullstelle hat, falls es eine Singularität gibt. \\
    \newline
    "$\Leftarrow$": \\
...
\end{thm}
\begin{proof}
    \underline{Fall 1:} $\patinf\oplus\patinf$: Es gilt nach Lemma~\ref{proj-tang} $T_{\patinf}=U$ für die unendlich ferne Gerade $U$. Da $\patinf$ der einzige Schnittpunkt von $U$ und $E$ ist, gilt $\patinf\oplus\patinf=\patinf$\\
...
\newline
Man spricht dabei von der \textit{Sekanten-Tangenten-Konstruktion}. Mit dieser Formel kann die Addition in beliebigen Körpern $\mathbb{F}$ berechnet werden, ohne auf die grafische Lösung zurückzugreifen.
...
Bei einer elliptischen Kurve $E$ über dem Körper $F_p$ kann auch ein Diskretes-Logarithmen-Problem konstruiert werden. Dazu wählen wir $P,Q \in E$ und ein $m \in \mathbb{N} \setminus \{0\}$. Das Diskrete-Logarithmen-Problem ist dann die Lösung der Gleichung $mP = Q$, wobei $P$ und $Q$ bekannt sind. 
...
Insgesamt haben elliptische Kurven durch ihre Eigenschaften eine zentrale Rolle in der Kryptologie erhalten und werden wohl auch in der Zukunft noch relevant bleiben.
\end{sloppypar}
\newpage
