...
Die drei x-Koordinaten müssen nicht voneinander verschieden sein, aber falls $x_i=x_j$ für $i\ne j$ gilt, dann auch $y_i=y_j$ und die affine Gerade ist genau die Tangente $T_P$ an $E$ im Punkt $P=(x_i,y_i)$.
...
\begin{equation*}
	2(kx_i+d)k=3x_i^2+a \Leftrightarrow k=\frac{3x_i^2+a}{2(kx_i+d)}=\frac{3x_i^2+a}{2y_i}
\end{equation*}
Das bedeutet, die Ableitung der affinen Gerade beschreibt genau die Ableitung unserer Kurve im Punkt $P$. Somit ist also auch $k$ die Steigung von $T_P$. Da $P$ sowohl auf $T_P$ als auch auf der affinen Gerade $y=kx+d$ liegt, gilt folglich Gleichheit.\\
