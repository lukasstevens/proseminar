...
\begin{defn} Es sei $\mathcal{A}$ eine Menge und $\mathcal{G}$ eine Teilmenge der Potenzmenge von $\mathcal{A}$, d.h. $\mathcal{G}\subseteq P(\mathcal{A})$.
Die Menge $\mathcal{A}$ nennt man die \textbf{Punktmenge} und die Menge $\mathcal{G}$ die \textbf{Geradenmenge} der affinen Ebene $(\mathcal{A},\mathcal{G})$, falls folgende vier Bedingungen erfüllt sind:
	\begin{enumerate}
...
\begin{center}
\begin{tikzpicture}[node distance=2.5cm,main node/.style={circle,fill=black,scale=0.75}]
...
Um in~\ref{affine-darstellung} eine affine Darstellung elliptischer Kurven herzuleiten, müssen wir $(\mathcal{P,G}) =$ \pgtwo nochmal betrachten.
Wir wählen dazu eine Gerade $U \in \mathcal{G}$ aus. Prinzipiell kann dazu jede Gerade gewählt werden. Es ist jedoch von Vorteil eine bestimmte Gerade zu wählen um das Rechnen mit der Weierstraßgleichung(\ref{weierstrass}) zu vereinfachen. \\
\newline
...
Im dreidimensionalen Raum ist das genau die $x,y$-Ebene.
% \begin{bsp}
%     \begin{tikzpicture}
%         [grid/.style={very thin,gray},axis/.style={->,black,thick}]
% 
%         %draw the axes
