...
\end{figure}
	\item[(A3)] Wegen (P3) gibt es vier verschiedene Punkte $A,B,C,D\in\mathcal{P}$, sodass nie drei davon auf derselben Gerade liegen.\\
		...
		Lagen zwei der vier Punkte auf $U$, o.B.d.A.\ sei dies $A,B$, so existieren zwei Geraden $\overline{A,C}$ und $\overline{B,D}$, die sich in einem Punkt $E$ schneiden, der nicht inzident zu $U=\overline{A,B}$ ist.
...
Um in Abschnitt~\ref{definition-ek} elliptische Kurven genau beschreiben zu können und in~\ref{affine-darstellung} eine affine Darstellung elliptischer Kurven herzuleiten, müssen wir $(\mathcal{P,G}) =$ \pgtwo nochmal betrachten.
Wir wählen eine Gerade $U \in \mathcal{G}$ aus, die wir aus der projektiven Ebene entfernen wollen.
...
\newline
Ein Beispiel für elliptische Kurven, für die eine solcher Isomorphismus existiert, sind die sogenannten anomalen elliptische Kurven. Eine elliptische Kurve ist \textbf{anomal}, wenn $\#E(\mathbb{F}_p) = p$, d.h.\ die Anzahl der Punkte von $E$ ist gleich der Anzahl der Elemente des Körpers $\mathbb{F}_p$. 
...
Insgesamt haben elliptische Kurven durch ihre Eigenschaften eine zentrale Rolle in der Kryptologie erhalten und werden wohl auch in der Zukunft eine Zentrale Rolle spielen.
\end{sloppypar}
\newpage
